...
function rowCountTotal(rows) {
  return (rows || []).reduce((sum, row) => sum + numberValue(row[1]), 0);
}
...
async function getVoteStats() {
  ...
  const recent = (resultAt(results, 5, []) || [])
    .map((item) => {
      try {
  ...
  let sources = parseHash(resultAt(results, 4, [])).sort((a, b) => b[1] - a[1]);
  const indexedSources = sourcesFromEvents(await getIndexedEvents());

  if (rowCountTotal(indexedSources) > rowCountTotal(sources)) {
    sources = indexedSources;
  }

  if (sources.length === 0) {
    sources = sourcesFromEvents(recent);
  }

  let locations = parseLocationHash(resultAt(results, 6, []));
  ...
  return {
    ...
    sources,
    locations,
    countries: parseHash(resultAt(results, 7, [])).sort((a, b) => b[1] - a[1]),
    locatedClicks,
